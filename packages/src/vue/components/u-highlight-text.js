export default {
    ...
    render(h, context) {
        ...
        if (!keyword || disabled) {
            return h(tagName, { class: [highlightWrapperClassName], ...context.data }, text)
        }
        ...
        children = children.map(currentText => {
            if (currentText.toLowerCase() === keyword.toLowerCase()) {
                return h('span', { class: [highlightClassName], style: { color: '#ed5f5f' } }, [currentText])
            }
            return currentText
        })

        return h(tagName, { class: [highlightWrapperClassName], ...context.data }, children)
    },
